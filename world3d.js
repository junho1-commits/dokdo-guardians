...
buildWorld=function(){const c=centers[state.stage];px=c[0];pz=c[1]+roomShapes[state.stage].spawn;yaw=0;pitch=-.04;camSnap=true;buildStations();if(playerKind!==choice.character){scene3d.remove(player3d);player3d=makeAvatar(choice.character);scene3d.add(player3d);playerKind=choice.character}if(window.BGM&&window.BGM.setRoom)window.BGM.setRoom(state.stage);};
...
const currentZone=zoneAt(px,pz);if(window.BGM&&window.BGM.setRoom&&window.BGM.currentRoom!==currentZone)window.BGM.setRoom(currentZone);
$('.interact').hidden=!nearest||activity.open||photoAlbum.open;$('.interact').textContent=nearest?`E · ${nearest.id==='gate'?'출구 잠금장치 조사':nearest.isNpc?(nearest.prompt||'이야기 듣기'):state.done.includes(nearest.id)?'다시 관찰하기':nearest.use?(selected===nearest.use?D.items[nearest.use][1]+' 사용하기':D.items[nearest.use][1]+' 꺼내는 방법'):'조사하기'}`:'';$('#target').textContent=nearest&&!activity.open?nearest.name:'';$('#heading').textContent=`${['북 N','동 E','남 S','서 W'][((Math.round(yaw/(Math.PI/2))%4)+4)%4]} · ${D.stages[currentZone].name}`;
