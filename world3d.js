...
const centers=[[-22,-22],[0,-22],[22,-22],[22,12],[0,12],[-22,12]];
...
for(const side of [-1,1]){rb(cx+side*7.15,.65,cz+.5,.38,1.3,24,wallColor);rb(cx+side*7.15,3.8,cz+.5,.38,.25,24,'#405e5d');for(const dz of [-10,-5,0,5,10])rb(cx+side*7.15,2.3,cz+dz,.4,3,.4,'#56716a');const glass=new T.MeshStandardMaterial({color:'#97c7cf',transparent:true,opacity:.12,roughness:.2,depthWrite:false});mesh(new T.BoxGeometry(.10,2.8,23),glass,cx+side*7.13,2.2,cz+.5);}
...
for(const npc of [].concat(D.npcs[i]||[])){const x=cx+npc.x,z=cz+npc.z,h=floorY(x,z),g=createNpc(npc.kind);g.position.set(x,h,z);g.rotation.y=Math.PI;scene3d.add(g);worldSolids.push({x:x-.6,z:z-.6,w:1.2,d:1.2});if(npc.kind==='guard')worldSolids.push({x:x+.6,z:z-.2,w:1.2,d:1.2});const tag=label(npc.name,x,(npc.fixed?3.9:3.15)+h,z,.67);npcObjects.push({npc:{...npc,x,z,chapter:i,id:'npc-'+npc.id,isNpc:true},g,tag,rest:Math.PI});}
...
canvas3d.onpointerdown=e=>{if(!active||activity.open||photoAlbum.open||e.pointerType==='mouse'&&![0,2].includes(e.button))return;e.preventDefault();canvas3d.focus();drag=[e.clientX,e.clientY];canvas3d.setPointerCapture(e.pointerId)};canvas3d.onpointermove=e=>{if(!drag)return;yaw-=(e.clientX-drag[0])*.004;pitch=Math.max(-.8,Math.min(.65,pitch-(e.clientY-drag[1])*.003));drag=[e.clientX,e.clientY]};for(const ev of ['pointerup','pointercancel','lostpointercapture'])canvas3d.addEventListener(ev,()=>drag=null);
...
const c=centers[state.stage],all=[...D.stages.slice(0,state.stage+1).flatMap(s=>s.nodes),{id:'gate',name:state.stage===5?'수호 메시지 장치':'출구 잠금장치',x:c[0],z:c[1]+11},...npcObjects.filter(o=>o.npc.chapter<=state.stage||freeRoam).map(o=>o.npc)];nearest=null;let dist=2.9;for(const n of all){let d=Math.hypot(n.x-px,n.z-pz);if(d<dist){nearest=n;dist=d}}
...
$('.game-tip').firstChild.textContent='WASD 이동 · Shift 달리기 · 좌/우클릭 회전 · CapsLock 방향 고정 · M 지도';
canvas3d.setAttribute('aria-label','독도 3D 탐험. 방향키 이동, 좌클릭 또는 우클릭 드래그로 자유 회전, 휠로 확대 축소. 1 일인칭, 2 자유 시점, 3 위에서 보기.');
const cameraHelp=itemHow;itemHow=function(){cameraHelp();activity.querySelector('.activity-body').insertAdjacentHTML('afterbegin','<p><b>자유 시점 조작</b><br>왼쪽 또는 오른쪽 마우스 버튼을 누르고 드래그하면 캐릭터 앞·옆·뒤를 볼 수 있어. 휠을 굴리면 확대·축소되고, 아주 가까이 가면 일인칭으로 바뀌어. Q/R로도 회전할 수 있어. CapsLock이나 방향 고정 버튼은 캐릭터를 카메라 방향에 맞춰 줘. 고정 중에도 화면을 회전할 수 있어. Shift를 누르며 이동하면 달려. M으로 지도를 열고 닫아. 터치 화면에서는 손가락으로 드래그해 봐.<br>1 일인칭 · 2 자유 시점 · 3 위에서 보기. 캐릭터는 걷는 방향을 바라봐.</p>')};$('#help-game').onclick=itemHow;
